...
public class ProjectOrderFragment extends BaseFoundFm implements ItemViewListener {
    ...
    private void LoadingData(final boolean isrefresh) {
        //创客商品订单
        String makerOrderListUrl = UrlAPI.getIImMakerOrderListUrl(UserInfoModel.getInstance().getId(), status, index, 103);
    ...
    public View getView(int id, View itemView, ViewGroup vg, Object data) {
        ...
        helper.updateViewPlay(model);
        if (model.status == 2 || model.status == 3|| model.status == 4) {
            itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    final Intent intent = new Intent(getContext(), PlayOrderDetailsActivity.class);
