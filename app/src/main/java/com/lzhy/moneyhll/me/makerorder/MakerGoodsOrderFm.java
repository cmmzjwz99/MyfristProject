...
public class MakerGoodsOrderFm extends BaseFoundFm implements ItemViewListener {
    private LongAdapter mAdapter;
    private List<MarkerOrderModel> itemdata;
    // 待支付1 ；待发货2；待收货3；已完结4；已失效5；未支付取消订单6； 退款取消订单7；
    private int status;
    private int index = 1;
    ...
    private void LoadingData(final boolean isrefresh) {
        //创客商品订单
        String makerOrderListUrl = UrlAPI.getMakerOrderListUrl(UserInfoModel.getInstance().getId(), status, index, 4);
        ...
        if (model.status == 2 || model.status == 3 || model.status == 4|| model.status == 8) {
            itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    Intent intent = new Intent(getContext(), MakerGoodsOrderDetailActivity.class);
                    Bundle extras = new Bundle();
                    extras.putSerializable("MakerGoodsModel", model);
